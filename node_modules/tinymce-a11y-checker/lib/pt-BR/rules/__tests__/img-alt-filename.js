"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault").default;

var _imgAltFilename = _interopRequireDefault(require("../img-alt-filename"));

let el;
beforeEach(() => {
  el = document.createElement("img");
  el.setAttribute("src", "/file.txt");
});
describe("test", () => {
  test("returns true if alt text is empty", () => {
    expect(_imgAltFilename.default.test(el)).toBeTruthy();
  });
  test("returns true if not img tag", () => {
    const div = document.createElement("div");
    expect(_imgAltFilename.default.test(el)).toBeTruthy();
  });
  test("returns true if alt text is empty", () => {
    el.setAttribute("alt", "");
    expect(_imgAltFilename.default.test(el)).toBeTruthy();
  });
  test("returns true if alt text is not filename", () => {
    el.setAttribute("alt", "some text");
    expect(_imgAltFilename.default.test(el)).toBeTruthy();
  });
  test("returns false if alt text is filename", () => {
    el.setAttribute("alt", "file.txt");
    expect(_imgAltFilename.default.test(el)).toBeFalsy();
  });
  test("returns false if alt text is filename with blank spaces", () => {
    el.setAttribute("alt", "file with blank.txt");
    expect(_imgAltFilename.default.test(el)).toBeFalsy();
  });
});
describe("data", () => {
  test("returns alt text", () => {
    el.setAttribute("alt", "some text");
    expect(_imgAltFilename.default.data(el).alt).toBe("some text");
  });
  test("returns empty alt text if no alt attribute", () => {
    expect(_imgAltFilename.default.data(el).alt).toBe("");
  });
  test("returns decorative true if el has empty alt attribute", () => {
    el.setAttribute("alt", "");
    expect(_imgAltFilename.default.data(el).decorative).toBeTruthy();
  });
});
describe("form", () => {
  test("returns the proper object", () => {
    expect(_imgAltFilename.default.form()).toMatchSnapshot();
  });
});
describe("update", () => {
  test("returns same element", () => {
    expect(_imgAltFilename.default.update(el, {})).toBe(el);
  });
  test("sets alt text to empty and role to 'presentation' if decorative", () => {
    _imgAltFilename.default.update(el, {
      decorative: true
    });

    expect(el.getAttribute("alt")).toBe("");
    expect(el.getAttribute("role")).toBe("presentation");
  });
  test("sets alt text and removes role if not decorative", () => {
    el.setAttribute("alt", "");
    el.setAttribute("role", "presentation");

    _imgAltFilename.default.update(el, {
      decorative: false,
      alt: "some text"
    });

    expect(el.getAttribute("alt")).toBe("some text");
    expect(el.hasAttribute("role")).toBeFalsy();
  });
  test("changes alt text if not decorative", () => {
    const text = "this is my text";
    el.setAttribute("alt", "thisismy.txt");
    expect(_imgAltFilename.default.update(el, {
      alt: text,
      decorative: false
    }).getAttribute("alt")).toBe(text);
  });
  test("removes role if not decorative", () => {
    const text = "this is my text";

    _imgAltFilename.default.update(el, {
      alt: text,
      decorative: false
    });

    expect(el.hasAttribute("role")).toBeFalsy();
    expect(el.getAttribute("alt")).toBe(text);
  });
  test("sets alt text to empty and role if decorative", () => {
    _imgAltFilename.default.update(el, {
      decorative: true
    });

    expect(el.getAttribute("role")).toBe("presentation");
    expect(el.getAttribute("alt")).toBe("");
  });
});
describe("message", () => {
  test("returns the proper message", () => {
    expect(_imgAltFilename.default.message()).toMatchSnapshot();
  });
});
describe("why", () => {
  test("returns the proper why message", () => {
    expect(_imgAltFilename.default.why()).toMatchSnapshot();
  });
});
describe("linkText", () => {
  test("returns the proper why message", () => {
    expect(_imgAltFilename.default.linkText()).toMatchSnapshot();
  });
});