...
import { initialOptions, getPlannerItems, getWeeklyPlannerItems, preloadSurroundingWeeks, scrollIntoPast, loadFutureItems, loadThisWeekItems, startLoadingAllOpportunities, toggleMissingItems, reloadWithObservee, getInitialOpportunities } from './actions';
...
function initializeCourseAndGroupColors(options) {
  ...
  options.env.STUDENT_PLANNER_COURSES = options.env.STUDENT_PLANNER_COURSES.map(dc => ({ ...dc,
    color: getCourseColor(dc, options.env)
  }));
  options.env.STUDENT_PLANNER_GROUPS = options.env.STUDENT_PLANNER_GROUPS.map(dg => ({ ...dg,
...
export function createPlannerApp() {
  if (!store.getState().weeklyDashboard) {
    ...
    store.dispatch(getPlannerItems(moment.tz(initializedOptions.env.timeZone).startOf('day'))).then(() => {
      store.dispatch(getInitialOpportunities());
    });
  } else {
    store.dispatch(getWeeklyPlannerItems(moment.tz(initializedOptions.env.timeZone).startOf('day'))).then(() => {
...
export function reloadPlannerForObserver(newObserveeId) {
  if (!initializedOptions) throw new Error('initializePlanner must be called before reloadPlannerForObserver'); // if observer is observing themselves, then we're not really observing

  const observeeId = !newObserveeId || newObserveeId === store.getState().currentUser.id ? null : newObserveeId;

  if (observeeId !== store.getState().selectedObservee && (store.getState().weeklyDashboard || ENV.FEATURES.observer_picker)) {
