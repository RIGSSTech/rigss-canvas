...
import styles from './styles'; // Import the <math-field> container and all
...
export default class EquationEditorModal extends Component {
  constructor(...args) {
    ...
    this.executeCommand = (cmd, advancedCmd) => {
      if (this.state.advanced) {
        ...
        this.setState(state => ({
          workingFormula: state.workingFormula + effectiveCmd
        }));
      } else {
    ...
    this.renderToggle = () => {
      ...
      const defaultToggle = /*#__PURE__*/React.createElement(Checkbox, {
      ...
      const tooltipToggle = /*#__PURE__*/React.createElement(Tooltip, {
    ...
    this.render = () => {
      ...
      }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement(MemoizedEquationEditorToolbar, {
  ...
  loadExistingFormula() {
    ...
    const selection = editor.selection.getContent(); // check if highlighted text is inline latex
  ...
  selectionIsLatex(selection) {
    return selection.startsWith('\\(') && selection.endsWith('\\)') || selection.startsWith('$$') && selection.endsWith('$$');
  }
