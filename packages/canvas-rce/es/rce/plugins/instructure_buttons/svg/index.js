...
export function buildSvg(settings, options = DEFAULT_OPTIONS) {
  ...
  g.appendChild(shape.cloneNode(true)); // Don't append an image if none has been selected
...
export function buildStylesheet() {
  ...
  return new Promise(resolve => resolve(fetch(url))).then(data => data.blob()).then(blob => convertFileToBase64(blob)).then(base64String => {
...
export function buildCheckerboard() {
  ...
  const children = [createSvgElement('rect', {
    fill: '#d9d9d9',
    x: '0',
    width: '8',
    height: '8',
