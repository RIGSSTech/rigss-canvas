...
export default function useDebouncedValue(currentValue, onChange) {
  const [immediateValue, setImmediateValue] = useState(currentValue); // The hook may have initially been called with currentValue
  ...
  const handleValueChange = event => {
    const {
      value
    } = event.target; // Immediately set local state for low-latency feedback
