...
exports.VIDEO_SIZE_OPTIONS = void 0;
exports.default = saveMediaRecording;
exports.saveClosedCaptions = saveClosedCaptions;

var _axios = _interopRequireDefault(require("axios"));

var _k5uploader = _interopRequireDefault(require("@instructure/k5uploader"));

var _FileSizeError = _interopRequireDefault(require("./shared/FileSizeError"));
...
exports.VIDEO_SIZE_OPTIONS = VIDEO_SIZE_OPTIONS;
...
function addUploaderFileCompleteEventListeners(uploader, context, file, done, onProgress) {
  uploader.addEventListener('K5.complete', async mediaServerMediaObject => {
    ...
    try {
      ...
      const canvasMediaObject = await _axios.default.post('/api/v1/media_objects', body, config);
      uploader.destroy();
      doDone(done, null, {
...
async function saveMediaRecording(file, rcsConfig, done, onProgress) {
  try {
    ...
    const mediaServerSession = await (0, _axios.default)({
      method: 'POST',
    ...
    const k5UploaderSession = new _k5uploader.default(session);
    ...
    if (st.isNew) {
      const p = new Promise((resolve, reject) => {
        if (maxBytes && st.file.size > maxBytes) {
          reject(new _FileSizeError.default({
            maxBytes,
